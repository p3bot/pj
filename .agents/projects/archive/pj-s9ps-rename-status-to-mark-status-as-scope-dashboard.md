...
8. Locked keys and meanings (value forms):
   ...
   - `mode` — `pj-driven` | `repo-driven` | `plain-files` only (no fourth `unknown`). Match product `deriveMode` for a *known* schema autoCommit: known true → always `pj-driven` (with or without a git-root — the no-repo “pj-driven planned” layout stays `pj-driven`, same as `pj scope list` / sync `sync_disabled:`); known false + git-root → `repo-driven`; known false + no git-root → `plain-files`. When autoCommit is unknown (schema missing/unparseable), do not call it `repo-driven` and do not emit `unknown`: use `plain-files` and ride stderr `config_unparseable:` (matches doctor’s refuse-to-judge-repo-driven rule). That unknown-schema case is the only intentional divergence from `pj scope list`’s `unknown` label — document both the three-label set and the stderr signal in Long help so agents do not read plain-files as “healthy host files” without checking stderr. `uncommitted` is non-zero only when mode is exactly `repo-driven`.
...
- Mode: do not use `schemaAutoCommit`'s nil→false trap for labelling. With a known schema, reuse the same mapping as `scopeadmin.deriveMode` (true → `pj-driven` even with no git-root; false + git-root → `repo-driven`; false without → `plain-files`). Only when autoCommit is unknown (unusable schema) diverge: emit `plain-files` and `uncommitted` 0, never `repo-driven` or `unknown`; `config_unparseable:` still rides stderr from reconcile. Long help must state the three-label set, the known-schema matrix (including pj-driven planned), and the stderr signal for unparseable config.
